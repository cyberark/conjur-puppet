...
master_container=$(docker ps | awk '/puppet\/puppetserver/{print $1}')
# Ignore errors since there might not be any certificates to clear
docker exec "$master_container" puppet cert clean "$hostname.cyber-ark.com" 2>/dev/null || true
...
gateway_ip="$(docker exec $master_container netstat -rn | grep "0.0.0.0.*UG" | awk '{print $2}')"
