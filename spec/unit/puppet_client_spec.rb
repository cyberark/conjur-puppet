...
-----END RSA PRIVATE KEY-----
        ...
        context 'when the certificate does not verify' do
          let(:cert_hostname) { 'not.localhost' }

          it 'error is raised' do
            expect { client.get 'test' }.to raise_error %r{verify failed}
          end
        end
      end
      ...
      context 'with mock conjur service', conjur: :mock do
        ...
        describe '#create_host' do
          it 'creates a host and returns the description hash' do
            allow(conjur_connection).to receive(:post) \
              .with(anything, nil, 'Authorization' => 'Token token="hostfactorytokenn"') \
            do |request_path|
              uri = URI request_path
              expect(uri.path).to eq '/host_factories/hosts'
              expect(URI.decode_www_form(uri.query)).to include(['id', 'test+plus!'],
                                                                ['annotations[puppet]', 'true'])
              http_ok '{ "api_key": "theapikey" }'
            end

            expect(subject.create_host('test+plus!',
                                       'hostfactorytokenn',
                                       annotations: { puppet: true })) \
        ...
        describe '#authenticate' do
          ...
          it 'correctly encodes username' do
            ...
            expect(subject.authenticate('host/puppettest',
                                        'the api key',
                                        'test')) \
